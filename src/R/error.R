...
getErrTable <- function(language,lib,pkg)
{
    ...
    fileName <- "ibmspsscfr.properties"
    ...
    if(!file.exists(errfile))
    {
        ...
        langPath <- file.path(rootPath, "lang", language)
        errfile <- file.path(langPath, fileName)
        warning("Can not find localized error message file. Using default ibmspsscfr.properties for English.")
    }
    if(!file.exists(errfile))
    {
...
options(warn=1)
spssError.reset <- function() last.SpssCfError <<- 0

translateMsg <- function(message)
{
    msg <- message
    for( m in ibmspsscf.generalErr )
    {
        if( m[1] == message )
        {
            msg <- m[2]
